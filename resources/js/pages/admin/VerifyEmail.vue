...
const { adminToken, fetchAdminProfile, getAdminClient } = useAuth();
const message = ref('');
const error = ref('');
...
onMounted(async () => {
    ...
    try {
        ...
        const { data } = await getAdminClient().get(`/auth/email/verify/${id}/${hash}?${params}`);
...
<template>
    <AuthPageShell title="Verify your email" subtitle="Confirm your email address to secure your admin account.">
        <div class="card-modern p-6 sm:p-8">
            ...
            <div v-else-if="error" class="mt-6 space-y-4">
                ...
                <RouterLink to="/admin/dashboard" class="btn-institutional inline-flex">Go to dashboard</RouterLink>
            </div>
        </div>
    </AuthPageShell>
</template>
